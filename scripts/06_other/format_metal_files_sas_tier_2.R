...
for (j in 1:length(pheno)) {
  ...
  if (pheno[j] %in% c("uc","ibd")) {
     tmp<-merge(tmp,tmp1[,c("ID","INFO","N","avgA2FREQ","N_CASES","avgA2FREQ_CASES","N_CONTROLS","avgA2FREQ_CONTROLS")],by.x="MarkerName",by.y="ID",sort=F,all.x=T)
  } else {
    ...
    tmp<-merge(tmp11,tmp1[,c("ID","INFO","N","avgA2FREQ","N_CASES","avgA2FREQ_CASES","N_CONTROLS","avgA2FREQ_CONTROLS")],by.x="MarkerName",by.y="ID",sort=F,all.x=T)
  ...
  fwrite(tmp,paste(path,"post_imputation/2022/analysis/metaanalysis/",pheno[j],"/",chr,"_",pheno[j],"_meta_sas_tier_2_noGC_PCs_firthse_formatted_A2_effect_and_alternative_with_per_sample_rate_and_avgA2freq.txt.gz",sep=""),
