...
for (j in 1:length(pheno)) {
  ...
  nmax<-max(tmp$N)
  tmp$rate_total_sample<-tmp$TotalSampleSize/nmax
  ...
  fwrite(tmp,paste(path,"post_imputation/2022/analysis/metaanalysis/",pheno[j],"/",chr,"_",pheno[j],"_meta_eur_tier_1_noGC_PCs_firthse_formatted_A2_effect_and_alternative_with_per_sample_rate_and_avgA2freq_withNeff.txt.gz",sep=""),
         col.names=T,row.names=F,quote=F,sep="\t")
  
  rm(tmp,freq,freq2,sample)
...
# "/software/R-4.3.1/bin/Rscript ${path_gwas}scripts/format_metal_files_IIBDGC.R ${ph} ${i} \
# > ${path_gwas}scripts/logs/format_metal_files_IIBDGC_${ph}_${j}_${chr}.Rout"
# done
# done
