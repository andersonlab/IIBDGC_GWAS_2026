...
if(chr=="X" && pheno!="cd") {
  array<-c("affymetrix6","humanomniexpress","gsa","ukbb","finngen","decode","interval_ibdbioresource")
} else if (chr=="X" && pheno=="cd") {
  array<-c("affymetrix6","gsa","ukbb","finngen","decode","interval_ibdbioresource")
} else if (pheno=="cd") {
...
for (j in 1:length(pheno)) {
  ...
  tmp<-fread(paste(path,"post_imputation/2022/analysis/metaanalysis/",pheno[j],"/chr",chr,"_eur_all_",pheno[j],"_meta_noGC_PCs_firthse_eur_tier_2_1_tmp.txt.gz",sep=""),head=F)
  ...
  for (jj in 1:length(array)) {
    ...
    if (array[jj]=="ukbb") {
      file_tmp11<-paste(path,"post_imputation/2022/analysis/stage_2_summary_statistics/",array[jj],"/",pheno[j],"/chr",chr,"_",array[jj],"_eur_step2_",pheno[j],"_eur_sex_PCs_firthse_",pheno[j],".regenie.gz",sep="")
    ...
    } else if (array[jj]=="interval_ibdbioresource") {
      file_tmp11<-paste(path,"post_imputation/2022/analysis/stage_2_summary_statistics/ibdbioresource/",pheno[j],"/",chr,"_",array[jj],"_eur_step2_",pheno[j],"_eur_sex_PCs_firthse_",pheno[j],".regenie.gz",sep="")
    }
  ...
  tmp1<-tmp1[which(tmp1$ID %in% tmp$MarkerName),]
  tmp1<-tmp1[!duplicated(tmp1),]
  
  print(paste("N duplicated variant IDs:",nrow(tmp1[duplicated(tmp1$ID),]),sep=""))
  
  ### OVERALL:
  
  N<-as.matrix(tmp1[,colnames(tmp1)[grep("N_[a-z0-9]*$",colnames(tmp1))],drop=F])
  Neff<-as.matrix(tmp1[,colnames(tmp1)[grep("Neff_[a-z0-9]*$",colnames(tmp1))],drop=F])
  
  A1FREQ<-as.matrix(tmp1[,colnames(tmp1)[grep("A1FREQ_[a-z0-9]*$",colnames(tmp1))],drop=F])
  INFO<-as.matrix(tmp1[,colnames(tmp1)[grep("INFO_[a-z0-9]*$",colnames(tmp1))],drop=F])
  ...
  # total number of cases - to report:
  N_CASES<-as.matrix(tmp1[,colnames(tmp1)[grep("N_CASES_",colnames(tmp1))],drop=F])
  N_CASES[is.na(A1FREQ) | is.na(INFO)]<-NA

  # total number of cases - to estimate allele frequency - excluding decode - no per disease-status allele frequency:
  N_CASES2<-as.matrix(tmp1[,colnames(tmp1)[grep("N_CASES_",colnames(tmp1))],drop=F])

  A1FREQ_CASES<-as.matrix(tmp1[,colnames(tmp1)[grep("A1FREQ_CASES",colnames(tmp1))],drop=F])

  N_CASES2[is.na(A1FREQ_CASES) | is.na(INFO)]<-NA

  A1FREQ_CASES2<-A1FREQ_CASES*N_CASES2
  A1FREQ_CASES2[is.na(N_CASES2) | is.na(INFO)]<-NA
  
  tmp1$avgA2FREQ_CASES<-NA
  tmp1$avgA2FREQ_CASES<-rowSums(A1FREQ_CASES2,na.rm=T)/rowSums(N_CASES2,na.rm=T)
  ...
  # total number of controls - to report:
  N_CONTROLS<-as.matrix(tmp1[,colnames(tmp1)[grep("N_CONTROLS_",colnames(tmp1))],drop=F])
  N_CONTROLS[is.na(A1FREQ) | is.na(INFO)]<-NA

  # total number of controls - to estimate allele frequency - excluding decode - no per disease-status allele frequency:
  N_CONTROLS2<-as.matrix(tmp1[,colnames(tmp1)[grep("N_CONTROLS_",colnames(tmp1))],drop=F])
  
  A1FREQ_CONTROLS<-as.matrix(tmp1[,colnames(tmp1)[grep("A1FREQ_CONTROLS",colnames(tmp1))],drop=F])

  N_CONTROLS2[is.na(A1FREQ_CONTROLS) | is.na(INFO)]<-NA

  A1FREQ_CONTROLS2<-A1FREQ_CONTROLS*N_CONTROLS2
  A1FREQ_CONTROLS2[is.na(N_CONTROLS2) | is.na(INFO)]<-NA
  
  tmp1$avgA2FREQ_CONTROLS<-NA
  tmp1$avgA2FREQ_CONTROLS<-rowSums(A1FREQ_CONTROLS2,na.rm=T)/rowSums(N_CONTROLS2,na.rm=T)
  ...
  fwrite(tmp,paste(path,"post_imputation/2022/analysis/metaanalysis/",pheno[j],"/",chr,"_",pheno[j],"_meta_eur_tier_2_noGC_PCs_firthse_formatted_A2_effect_and_alternative_with_per_sample_rate_and_avgA2freq_withNeff.txt.gz",sep=""),
