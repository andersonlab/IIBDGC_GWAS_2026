...
# dataset<-"GCST90018947"
# dataset<-"GCST90002322"
...
gsmr_data$'P-value'<-as.numeric(gsmr_data$'P-value')
gsmr_data<-gsmr_data[which(gsmr_data$'P-value'<=p_threshold),]
dim(gsmr_data)
# [1] 13662 11
...
dim(gsmr_data)
#  [1] 13662 11 - GCST004988
# [1] 929  11 - GCST005531
# [1] 33749  11 - GCST90029070
...
# if the number of variants is >40K randomly select a subset of 20K gwas-significant variants
if(nrow(gsmr_data)>40000) {

    set.seed(42)
    gsmr_data<-gsmr_data[sample(1:nrow(gsmr_data),20000,replace=F),]
    gsmr_data_ibd_traits<-gsmr_data_ibd_traits[which(gsmr_data_ibd_traits$SNP %in% gsmr_data$SNP),]
    gsmr_data<-gsmr_data[match(gsmr_data_ibd_traits$SNP,gsmr_data$SNP),]
...
if (length(snp_coeff_id)>10) {
    ...
    gsmr_data$bzx<-bzx
    gsmr_data$bzx_se<-bzx_se
    gsmr_data$bzx_pval<-bzx_pval
    gsmr_data$bzy_pval<-bzy_pval
    ...
    if (nrow(gsmr_data[which(gsmr_data$'P-value'<=p_threshold),])<10) {
        ...
        data$pvalue_on_risk_factor<-paste(gsmr_data$bzx_pval[which(gsmr_data$SNP %in% gsmr_data$SNP[gsmr_results$used_index])],collapse="|")

        # extract pvalue of the SNPs on the disease:
        data$pvalue_on_disease<-paste(gsmr_data$bzy_pval[which(gsmr_data$SNP %in% gsmr_data$SNP[gsmr_results$used_index])],collapse="|")

        # # retrieve the list of SNPs used in the analysis - BEFORE excluding the outliers:
        ...
        geom_point(size = 3) + ylab(paste("Beta ",toupper(pheno)," susceptibility (bzy)",sep="")) + 
        geom_abline(slope=data$bxy,colour="blue",lty=2) + 
        xlab(paste("Beta ",dataset," (bzx)",sep=""))  + 
        ...
        ggsave(
            paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/plots/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,".png",sep=""),
            p,
            width = 45,
            height = 45,
            dpi = 400,
        ...
        # save main results:
        fwrite(data,paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/results/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,".tsv.gz",sep=""),col.names=T,row.names=F,quote=F,sep="\t")

        summary_data$SNP<-list_snps

        # save results for the plot
        fwrite(summary_data,paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/plots/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,"_data_source.tsv.gz",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
