...
# only retain those listed as index variants:
list_index<-fread(paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/forward_regression/list_tier_2_new_and_old_index_gwas_variants_conditional_on_known_index_gwas_fm_2023_with_iibdgc_tier1_tier2_eas_sas_summary_stats_plus_unsupervised_conditional.tsv",sep=""),head=T)
dim(list_index)
# [1] 744 230

gsmr_data_ibd_traits<-gsmr_data_ibd_traits[which(gsmr_data_ibd_traits$SNP %in% list_index$MarkerName),]
# [1] 205  11

# retain only strong instruments - as listed in summary table:
...
if (length(snp_coeff_id)>10) {
    ...
    if (nrow(gsmr_data_ibd_traits[which(gsmr_data_ibd_traits$'P-value'<=p_threshold),])<10) {
        ...
        ggsave(
            paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/plots/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,"_reverse_supervised.png",sep=""),
            p,
            width = 45,
            height = 45,
            dpi = 400,
        ...
        # save main results:
        fwrite(data,paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/results/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,"_reverse_supervised.tsv.gz",sep=""),col.names=T,row.names=F,quote=F,sep="\t")

        summary_data$SNP<-list_snps
        
        # save results for the plot
        fwrite(summary_data,paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/plots/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,"_data_source_reverse_supervised.tsv.gz",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
