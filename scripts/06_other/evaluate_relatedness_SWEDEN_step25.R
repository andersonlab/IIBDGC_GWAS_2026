...
path_variants=${path_gwas}pre_imputation/QC/relatedness/list_common_variants_among_cohorts_step25_sweden
...
--make-bed \
--out ${path_gwas}pre_imputation/QC/relatedness/${i}_subset_sweden_step25
done
...
wc -l ${path_gwas}pre_imputation/QC/relatedness/*_subset_sweden_step25.fam
# 1374 /path/to/ibdgwas/IIBDGC/pre_imputation/QC/relatedness/sweden_gsa_subset_sweden_step25.fam
# 1258 /path/to/ibdgwas/IIBDGC/pre_imputation/QC/relatedness/swedish_uc_old_gwas_subset_sweden_step25.fam
# 2632 total

wc -l ${path_gwas}pre_imputation/QC/relatedness/*_subset_sweden_step25.bim
# 62063  OK
...
for (i in 1:nrow(dat)){
  dat[i,1]<-paste(path,"pre_imputation/QC/relatedness/",cohorts[i],"_subset_sweden_step25.bed",sep="")
  dat[i,2]<-paste(path,"pre_imputation/QC/relatedness/",cohorts[i],"_subset_sweden_step25.bim",sep="")
  dat[i,3]<-paste(path,"pre_imputation/QC/relatedness/",cohorts[i],"_subset_sweden_step25.fam",sep="")
}

write.table(dat,paste(path,"pre_imputation/QC/relatedness/list_cohorts_tomerge_sweden_step25.txt",sep=""),col.names=F,row.names=F,quote=F,sep="\t")
...
/path/to/software/plink_linux_x86_64_20181202/./plink \
--bfile ${path_gwas}pre_imputation/QC/relatedness/swedish_uc_old_gwas_subset_sweden_step25 \
--allow-no-sex \
--merge-list ${path_gwas}pre_imputation/QC/relatedness/list_cohorts_tomerge_sweden_step25.txt \
--make-bed --out ${path_gwas}pre_imputation/QC/relatedness/sweden_merged_step25
# 62063 variants and 2632 people pass filters and QC.
# Among remaining phenotypes, 1312 are cases and 1320 are controls.
...
-e ${path_gwas}pre_imputation/QC/relatedness/logs/stderr_king_sweden_step25 \
-o ${path_gwas}pre_imputation/QC/relatedness/logs/stdout_king_sweden_step25 \
"/path/to/software/./king \
-b ${path_gwas}pre_imputation/QC/relatedness/sweden_merged_step25.bed \
--related --cpus 8 --prefix ${path_gwas}pre_imputation/QC/relatedness/sweden_merged_step25"
# Job <186307> is submitted to queue <normal>.

less ${path_gwas}pre_imputation/QC/relatedness/logs/stdout_king_sweden_step25

# Relationship summary (total relatives: 0 by pedigree, 55 by inference)
# MZ      PO      FS      2nd
# =====================================================
...
famall<-read.table(paste(path,"pre_imputation/QC/relatedness/sweden_merged_step25.fam",sep=""),head=F)
...
#               Group.1 x.1 x.2
# 1          sweden_gsa 979 395
# 2 swedish_uc_old_gwas 341 917

colnames(dat)[5]<-"sex"
aggregate(as.factor(dat$sex), by=list(dat$cohort), FUN=summary)
#               Group.1 x.1 x.2
# 1          sweden_gsa 795 579
# 2 swedish_uc_old_gwas 693 565

table(dat$V1==dat$V2)
# TRUE 
# 2632 

kin<-read.table(paste(path,"pre_imputation/QC/relatedness/sweden_merged_step25.kin0",sep=""),head=T)
table(kin$InfType)
# Dup/MZ     FS     PO 
# 16     13     26 
...
write.table(x,paste(path,"pre_imputation/QC/relatedness/sweden_dup_summary_table_step25.tsv",sep=""),col.names=T,row.names=T,sep="\t",quote=F)
...
old_gwas<-c("swedish_uc_old_gwas")
new_gwas<-c("sweden_gsa")
...
write.table(data_remove,paste(path,"pre_imputation/QC/relatedness/sweden_data_remove_step25",sep=""),col.names=T,row.names=T,sep="\t",quote=F)
write.table(data_remove_tmp,paste(path,"pre_imputation/QC/relatedness/list_sweden_data_remove_step25",sep=""),col.names=T,row.names=F,sep="\t",quote=F)

write.table(data_inconsist,paste(path,"pre_imputation/QC/relatedness/sweden_data_inconsist_step25",sep=""),col.names=T,row.names=T,sep="\t",quote=F)
write.table(aggregate(as.factor(dat$pheno), by=list(dat$cohort), FUN=summary),paste(path,"pre_imputation/QC/relatedness/sweden_case_control_pre_per_study_relatedness_step25",sep=""),col.names=T,row.names=F,sep="\t",quote=F)
