...
for (i in 1:length(pheno)) {
  ...
  # add chisq
  old2<-fread(paste0(path_gwas,"summary_files/summary_stats_",pheno[i],"_delange_sorted_noheader_chr_pos_b37_pval"),head=F)
...
p1<-ggplot(new[which(new$avgA2FREQ_CASES>=0.01 & new$avgA2FREQ_CASES<=0.99 & new$pvalue<0.01),], aes(x=-log10(pvalue), y=(-log10(pvalue_new)))) + 
              ...
              ggtitle(paste(toupper(pheno[i]),"P-value variants MAF 0.01, delange pval < 0.01",pheno[i])) + geom_smooth(method='lm', color="darkgrey",linetype="dashed", size=0.8)

ggsave(paste0("~/git/IIBDGC_GWAS/plots/metaanalysis/pvalues_delange_vs_iibdgc_",release,"_",pheno[i],"_maf_0.01_delange_pval_0.1.pdf",sep=""),
    p1,
    width = 10,
    height = 10,
    dpi = 200,
...
new_significant<-new[which(new$pvalue<5E-8),]
...
p1<-ggplot(new_significant, aes(x=chisq, y=chisq_new)) + 
              geom_point(shape=20) + xlim(0,maxlim) + ylim(0,maxlim) + 
              geom_abline(intercept = 0, slope = 1, color="red",linetype="dashed", size=0.5) +
              ggtitle(paste(toupper(pheno[i]),"Chisq signif variants new iibdgc",release,pheno[i],"vs delange\nslope chisq_new ~ chisq_old",coef(mod)[2])) + geom_smooth(method='lm', color="darkgrey",linetype="dashed", size=0.8)

ggsave(paste0("~/git/IIBDGC_GWAS/plots/metaanalysis/chisq_delange_vs_iibdgc_",release,"_",pheno[i],"_signif.pdf",sep=""),
    p1,
    width = 10,
    height = 10,
    dpi = 200,
