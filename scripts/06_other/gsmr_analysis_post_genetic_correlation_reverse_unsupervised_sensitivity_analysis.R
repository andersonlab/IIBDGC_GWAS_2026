...
summary_data<-fread(paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/plots/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,"_data_source_reverse_unsupervised.tsv.gz",sep=""),head=T)

# load results with excluded (labelled as pleiotropic) variants:
data<-fread(paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/results/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,"_reverse_unsupervised.tsv.gz",sep=""),head=T)
...
# all selected for being strong instruments and relatively freq 
...
if (length(snp_coeff_id)>10) {
    ...
    fwrite(data,paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/results/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,"_reverse_unsupervised_with_without_heidi.tsv.gz",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
    ...
    for (n in 1:nrow(gsmr_data)) {
        ...
        snpfreq<-gsmr_data_ibd_traits$A1FREQ[-n]             # allele frequencies of the SNPs

        bzx<-gsmr_data_ibd_traits$BETA[-n]     # effects of the instruments on risk factor
        bzx_se<-gsmr_data_ibd_traits$SE[-n]       # standard errors of bzx
        bzx_n<-gsmr_data_ibd_traits$N[-n]          # GWAS sample size for the risk factor
        bzx_pval<-gsmr_data_ibd_traits$'P-value'[-n]   # p-values for bzx

        bzy<-gsmr_data$BETA[-n]    # SNP effects on the disease
        bzy_se<-gsmr_data$SE[-n]    # standard errors of bzy
        bzy_pval<-gsmr_data$'P-value'[-n]    # p-values for bzy
    ...
    # save main results:
    fwrite(data,paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/results/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,"_reverse_unsupervised_sensitivity_test.tsv.gz",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
