...
# "/software/R-4.3.1/bin/Rscript ${path_gwas}scripts/format_files_eur_tier_2_for_coloc.R ${ph} ${chr} \
# > ${path_gwas}scripts/logs/format_files_eur_tier_2_for_coloc_${ph}_${chr}.Rout"
# done
# done

# MEM=6000
# bsub -Is -M"$MEM" -R"select[mem>$MEM] rusage[mem=$MEM]" -m "modern_hardware" -G ibdgwas -q "yesterday" /software/bin/R-4.3.1
...
for (j in 1:length(pheno)){
  ...
  tmp<-fread(paste(path,"post_imputation/2022/analysis/metaanalysis/",pheno[j],"/",chr,"_",pheno[j],"_meta_eur_tier_2_noGC_PCs_firthse_formatted_A2_effect_and_alternative_with_per_sample_rate_and_avgA2freq.txt.gz",sep=""),head=T)
  ...
  print(nrow(tmp))
  tmp<-tmp[which(tmp$rate_total_sample>=0.50),]
  print(nrow(tmp))
  ...
  write.table(tmp,paste("/path/to/project",pheno[j],"_chr",chr,"_summary_stats_metal_nogc_June2023_eur_tier2.tsv",sep=""),
