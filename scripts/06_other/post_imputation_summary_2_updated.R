...
# GET SUMMARY DATA FOR ALL CHROMOSOMES TOGETHER, AFTER EXCLUDING THOSE GENOTYPED VARIANTS WITH RSQ <0.25 AND RUN IMPUTATION AGAIN
...
for (j in 1:length(ancestry)) {
  ...
  write.table(maf_dist,paste(path,"imputed/",study,"/",ancestry[j],"/plots/",study,"_",ancestry[j],"_maf_distribution_updated.tsv",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
  write.table(maf_dist_2,paste(path,"imputed/",study,"/",ancestry[j],"/plots/",study,"_",ancestry[j],"_maf_distribution_variants_rsq_0.4_updated.tsv",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
  write.table(rsq_dist,paste(path,"imputed/",study,"/",ancestry[j],"/plots/",study,"_",ancestry[j],"_rsq_distribution_updated.tsv",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
  write.table(gtImp,paste(path,"imputed/",study,"/",ancestry[j],"/plots/",study,"_",ancestry[j],"_genotyped_imputed_variants_distribution_updated.tsv",sep=""),col.names=T,row.names=F,quote=F,sep="\t")

  
  ##################################################
  # plot EmpR and EmpRsq  vs MAF for all chrs (similar to Kyle plots)
  
  p1<-ggplot(df, aes(x = MAF, y = EmpR)) + ggtitle(study) + ylim(-0.1,1) + ylab("Empirical R") + xlab("MAF") +
    geom_point(alpha = 0.3,shape=20,size=1)
  
  p2<-ggplot(df, aes(x = MAF, y = EmpRsq)) + ggtitle(study) + ylim(-0.1,1) + ylab("Empirical Rsq") + xlab("MAF") +
    geom_point(alpha = 0.3,shape=20,size=1)
  
  pdf(paste(path,"imputed/",study,"/",ancestry[j],"/plots/",study,"_",ancestry[j],"_maf_EmpRsq_updated.pdf",sep=""),width = 14, height = 5)
  ...
  pdf(paste(path,"imputed/",study,"/",ancestry[j],"/plots/",study,"_",ancestry[j],"_maf_EmpRsq_mean_updated.pdf",sep=""),width = 7, height = 4)
  print(p1)
  dev.off()
  
  
  write.table(sum_df,paste(path,"imputed/",study,"/",ancestry[j],"/plots/",study,"_",ancestry[j],"_maf_EmpRsq_mean_updated.tsv",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
