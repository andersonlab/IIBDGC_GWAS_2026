...
for (i in 1:length(pheno)) {
  ...
  # delange:
  old<-fread(paste("/path/to/ibdgwas/IIBDGC/summary_files/summary_stats_",pheno[i],"_liu_sorted_noheader_chr_pos_b37_pval_lifted_hg38.gz",sep=""),head=F)
  colnames(old)<-c("chr","position_b38","end","ID_b37","pvalue")
  old<-old[,c("chr","position_b38","ID_b37","pvalue")]


  # add chisq
  old2<-fread(paste0(path_gwas,"summary_files/summary_stats_",pheno[i],"_liu_sorted_noheader_chr_pos_b37_pval"),head=F)
...
ggsave(paste0("~/git/IIBDGC_GWAS/plots/metaanalysis/pvalues_liu_vs_iibdgc_",release,"_",pheno[i],"_maf_0.01_liu_pval_0.1.pdf",sep=""),
    p1,
    width = 10,
    height = 10,
    dpi = 200,
