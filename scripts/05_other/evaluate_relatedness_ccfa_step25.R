...
#
# EVALUATE RELATEDNESS BETWEEN ccfa SAMPLES
...
path_variants=${path_gwas}pre_imputation/QC/relatedness/list_common_variants_among_cohorts_step25_ccfa
...
--make-bed \
--out ${path_gwas}pre_imputation/QC/relatedness/${i}_subset_ccfa_step25
done
...
wc -l ${path_gwas}pre_imputation/QC/relatedness/*_subset_ccfa_step25.fam
# 2166 /path/to/ibdgwas/IIBDGC/pre_imputation/QC/relatedness/ccfa_gsa_subset_ccfa_step25.fam
# 2831 /path/to/ibdgwas/IIBDGC/pre_imputation/QC/relatedness/lewis_sparc_gsa_subset_ccfa_step25.fam
# 4997 total

wc -l ${path_gwas}pre_imputation/QC/relatedness/*_subset_ccfa_step25.bim
# 583991
...
for (i in 1:nrow(dat)){
  dat[i,1]<-paste(path,"pre_imputation/QC/relatedness/",cohorts[i],"_subset_ccfa_step25.bed",sep="")
  dat[i,2]<-paste(path,"pre_imputation/QC/relatedness/",cohorts[i],"_subset_ccfa_step25.bim",sep="")
  dat[i,3]<-paste(path,"pre_imputation/QC/relatedness/",cohorts[i],"_subset_ccfa_step25.fam",sep="")
}

write.table(dat,paste(path,"pre_imputation/QC/relatedness/list_cohorts_tomerge_ccfa_step25.txt",sep=""),col.names=F,row.names=F,quote=F,sep="\t")
...
/path/to/software/plink_linux_x86_64_20181202/./plink \
--bfile ${path_gwas}pre_imputation/QC/relatedness/ccfa_gsa_subset_ccfa_step25 \
--allow-no-sex \
--merge-list ${path_gwas}pre_imputation/QC/relatedness/list_cohorts_tomerge_ccfa_step25.txt \
--make-bed --out ${path_gwas}pre_imputation/QC/relatedness/ccfa_merged_step25

# 583991 variants and 4997 people pass filters and QC.
# Among remaining phenotypes, 4997 are cases and 0 are controls.
...
-e ${path_gwas}pre_imputation/QC/relatedness/logs/stderr_king_ccfa_step25 \
-o ${path_gwas}pre_imputation/QC/relatedness/logs/stdout_king_ccfa_step25 \
"/path/to/software/./king \
-b ${path_gwas}pre_imputation/QC/relatedness/ccfa_merged_step25.bed \
--related --cpus 8 --prefix ${path_gwas}pre_imputation/QC/relatedness/ccfa_merged_step25"
...
table(dat$cohort)
# ccfa_gsa lewis_sparc_gsa 
# 2166            2831

# double check all samples included in exercise
famall<-read.table(paste(path,"pre_imputation/QC/relatedness/ccfa_merged_step25.fam",sep=""),head=F)
...
#             Group.1 x.1 x.2
# 1        ccfa_gsa 2166
# 2 lewis_sparc_gsa 2831

colnames(dat)[5]<-"sex"
aggregate(as.factor(dat$sex), by=list(dat$cohort), FUN=summary)
#               Group.1 x.1 x.2
# 1        ccfa_gsa    1  992 1173
# 2 lewis_sparc_gsa    6 1274 1551

table(dat$V1==dat$V2)
# TRUE 
# 4997 

kin<-read.table(paste(path,"pre_imputation/QC/relatedness/ccfa_merged_step25.kin0",sep=""),head=T)
table(kin$InfType)
# Dup/MZ     FS     PO 
# 2166     16     16 
...
#                 ccfa_gsa lewis_sparc_gsa
# ccfa_gsa               0            2166
# lewis_sparc_gsa     2166               0


write.table(x,paste(path,"pre_imputation/QC/relatedness/ccfa_dup_summary_table_step25.tsv",sep=""),col.names=T,row.names=T,sep="\t",quote=F)
...
old_gwas<-c("ccfa_gsa")# force old release to be excluded, all included in ccfa_new
new_gwas<-c("lewis_sparc_gsa")
...
write.table(data_remove,paste(path,"pre_imputation/QC/relatedness/ccfa_data_remove_step25",sep=""),col.names=T,row.names=T,sep="\t",quote=F)
write.table(data_remove_tmp,paste(path,"pre_imputation/QC/relatedness/list_ccfa_data_remove_step25",sep=""),col.names=T,row.names=F,sep="\t",quote=F)

write.table(data_inconsist,paste(path,"pre_imputation/QC/relatedness/ccfa_data_inconsist_step25",sep=""),col.names=T,row.names=T,sep="\t",quote=F)
write.table(aggregate(as.factor(dat$pheno), by=list(dat$cohort), FUN=summary),paste(path,"pre_imputation/QC/relatedness/ccfa_case_control_pre_per_study_relatedness_step25",sep=""),col.names=T,row.names=F,sep="\t",quote=F)
