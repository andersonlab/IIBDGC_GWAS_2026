...
for (j in 1:length(ancestry)) {
  ...
  for (i in 1:23) {
    ...
    if(i==23) {
      tmp_file<-paste(path,"imputed/",study,"/2022/",ancestry[j],"/chrX.info.gz",sep="")
    } else {
      tmp_file<-paste(path,"imputed/",study,"/2022/",ancestry[j],"/chr",i,".info.gz",sep="")
    }
    ...
    if (file.exists(tmp_file)) {
      ...
      ##################################################
      # create list of samples for analysis:
      
      analysis_tmp<-tmp[which(tmp$MAF>=0.001 & tmp$Rsq>=0.4),]
      
      if(i==1){
        analysis<-analysis_tmp
      } else {
        analysis<-rbind(analysis,analysis_tmp)
      }
  ...
  write.table(maf_dist,paste(path,"imputed/",study,"/2022/",ancestry[j],"/plots/",study,"_",ancestry[j],"_maf_distribution_updated.tsv",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
  write.table(maf_dist_2,paste(path,"imputed/",study,"/2022/",ancestry[j],"/plots/",study,"_",ancestry[j],"_maf_distribution_variants_rsq_0.4_updated.tsv",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
  write.table(rsq_dist,paste(path,"imputed/",study,"/2022/",ancestry[j],"/plots/",study,"_",ancestry[j],"_rsq_distribution_updated.tsv",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
  write.table(gtImp,paste(path,"imputed/",study,"/2022/",ancestry[j],"/plots/",study,"_",ancestry[j],"_genotyped_imputed_variants_distribution_updated.tsv",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
  ...
  pdf(paste(path,"imputed/",study,"/2022/",ancestry[j],"/plots/",study,"_",ancestry[j],"_maf_EmpRsq_updated.pdf",sep=""),width = 14, height = 5,onefile=FALSE)
  ...
  pdf(paste(path,"imputed/",study,"/2022/",ancestry[j],"/plots/",study,"_",ancestry[j],"_maf_EmpRsq_mean_updated.pdf",sep=""),width = 7, height = 4,onefile=FALSE)
  print(p1)
  dev.off()

  write.table(sum_df,paste(path,"imputed/",study,"/2022/",ancestry[j],"/plots/",study,"_",ancestry[j],"_maf_EmpRsq_mean_updated.tsv",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
