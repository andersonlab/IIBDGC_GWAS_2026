...
#
# EVALUATE RELATEDNESS BETWEEN mccauley SAMPLES
...
path_variants=${path_gwas}pre_imputation/QC/relatedness/list_common_variants_among_cohorts_step25_mccauley
...
--make-bed \
--out ${path_gwas}pre_imputation/QC/relatedness/${i}_subset_mccauley_step25
done
...
wc -l ${path_gwas}pre_imputation/QC/relatedness/*_subset_mccauley_step25.fam
# 776 /path/to/ibdgwas/IIBDGC/pre_imputation/QC/relatedness/mccauley_gsa_subset_mccauley_step25.fam
# 1603 /path/to/ibdgwas/IIBDGC/pre_imputation/QC/relatedness/mccauley_new_gsa_subset_mccauley_step25.fam
# 2379 total

wc -l ${path_gwas}pre_imputation/QC/relatedness/*_subset_mccauley_step25.bim
# 542092  OK
...
for (i in 1:nrow(dat)){
  dat[i,1]<-paste(path,"pre_imputation/QC/relatedness/",cohorts[i],"_subset_mccauley_step25.bed",sep="")
  dat[i,2]<-paste(path,"pre_imputation/QC/relatedness/",cohorts[i],"_subset_mccauley_step25.bim",sep="")
  dat[i,3]<-paste(path,"pre_imputation/QC/relatedness/",cohorts[i],"_subset_mccauley_step25.fam",sep="")
}

write.table(dat,paste(path,"pre_imputation/QC/relatedness/list_cohorts_tomerge_mccauley_step25.txt",sep=""),col.names=F,row.names=F,quote=F,sep="\t")
...
/path/to/software/plink_linux_x86_64_20181202/./plink \
--bfile ${path_gwas}pre_imputation/QC/relatedness/mccauley_new_gsa_subset_mccauley_step25 \
--allow-no-sex \
--merge-list ${path_gwas}pre_imputation/QC/relatedness/list_cohorts_tomerge_mccauley_step25.txt \
--make-bed --out ${path_gwas}pre_imputation/QC/relatedness/mccauley_merged_step25

# 542092 variants and 2379 people pass filters and QC.
# Among remaining phenotypes, 2144 are cases and 235 are controls.
...
-e ${path_gwas}pre_imputation/QC/relatedness/logs/stderr_king_mccauley_step25 \
-o ${path_gwas}pre_imputation/QC/relatedness/logs/stdout_king_mccauley_step25 \
"/path/to/software/./king \
-b ${path_gwas}pre_imputation/QC/relatedness/mccauley_merged_step25.bed \
--related --cpus 8 --prefix ${path_gwas}pre_imputation/QC/relatedness/mccauley_merged_step25"
...
table(dat$cohort)
# mccauley_gsa mccauley_new_gsa 
# 776             1603

# double check all samples included in exercise
famall<-read.table(paste(path,"pre_imputation/QC/relatedness/mccauley_merged_step25.fam",sep=""),head=F)
...
#             Group.1 x.1 x.2
# 1     mccauley_gsa    0  776
# 2 mccauley_new_gsa  235 1368

colnames(dat)[5]<-"sex"
aggregate(as.factor(dat$sex), by=list(dat$cohort), FUN=summary)
#               Group.1 x.1 x.2
# 1     mccauley_gsa   0 399 377
# 2 mccauley_new_gsa  11 823 769

table(dat$V1==dat$V2)
# TRUE 
# 2379 

kin<-read.table(paste(path,"pre_imputation/QC/relatedness/mccauley_merged_step25.kin0",sep=""),head=T)
table(kin$InfType)
# Dup/MZ     FS     PO 
# 771     23     46  
...
#                 mccauley_gsa mccauley_new_gsa
# mccauley_gsa                0              771
# mccauley_new_gsa          771                0


write.table(x,paste(path,"pre_imputation/QC/relatedness/mccauley_dup_summary_table_step25.tsv",sep=""),col.names=T,row.names=T,sep="\t",quote=F)
...
old_gwas<-c("mccauley_gsa")# force old release to be excluded, all included in mccauley_new
new_gwas<-c("mccauley_new_gsa")
...
write.table(data_remove,paste(path,"pre_imputation/QC/relatedness/mccauley_data_remove_step25",sep=""),col.names=T,row.names=T,sep="\t",quote=F)
write.table(data_remove_tmp,paste(path,"pre_imputation/QC/relatedness/list_mccauley_data_remove_step25",sep=""),col.names=T,row.names=F,sep="\t",quote=F)

write.table(data_inconsist,paste(path,"pre_imputation/QC/relatedness/mccauley_data_inconsist_step25",sep=""),col.names=T,row.names=T,sep="\t",quote=F)
write.table(aggregate(as.factor(dat$pheno), by=list(dat$cohort), FUN=summary),paste(path,"pre_imputation/QC/relatedness/mccauley_case_control_pre_per_study_relatedness_step25",sep=""),col.names=T,row.names=F,sep="\t",quote=F)
