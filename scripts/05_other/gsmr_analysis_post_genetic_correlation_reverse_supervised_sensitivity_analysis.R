...
summary_data<-fread(paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/plots/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,"_data_source_reverse_supervised.tsv.gz",sep=""),head=T)

# load results with excluded (labelled as pleiotropic) variants:
data<-fread(paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/results/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,"_reverse_supervised.tsv.gz",sep=""),head=T)
...
if (length(snp_coeff_id)>10) {
    ...
    fwrite(data,paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/results/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,"_reverse_supervised_with_without_heidi.tsv.gz",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
    ...
    # save main results:
    fwrite(data,paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/annotation/ldscore/analysis_june2024/genetic_correlation/mendelian_randomization/results/gsmr_eur_tier2_",dataset,"_",pheno,"_",p_threshold,"_reverse_supervised_sensitivity_test.tsv.gz",sep=""),col.names=T,row.names=F,quote=F,sep="\t")
