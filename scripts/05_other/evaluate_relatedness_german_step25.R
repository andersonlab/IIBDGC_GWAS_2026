...
path_variants=${path_gwas}pre_imputation/QC/relatedness/list_common_variants_among_cohorts_step25_german
...
--allow-no-sex \
--make-bed \
--out ${path_gwas}pre_imputation/QC/relatedness/${i}_subset_german_step25
done
...
wc -l ${path_gwas}pre_imputation/QC/relatedness/*_subset_german_step25.fam
# 857 /path/to/ibdgwas/IIBDGC/pre_imputation/QC/relatedness/franke_gsa_subset_german_step25.fam
# 2761 /path/to/ibdgwas/IIBDGC/pre_imputation/QC/relatedness/german_affy6_old_gwas_subset_german_step25.fam
# 13835 /path/to/ibdgwas/IIBDGC/pre_imputation/QC/relatedness/kiel_austria_sibdcs_gsa_subset_german_step25.fam
# 17453 total

wc -l ${path_gwas}pre_imputation/QC/relatedness/*_subset_german_step25.bim
# 67623  OK
...
for (i in 1:nrow(dat)){
  dat[i,1]<-paste(path,"pre_imputation/QC/relatedness/",cohorts[i],"_subset_german_step25.bed",sep="")
  dat[i,2]<-paste(path,"pre_imputation/QC/relatedness/",cohorts[i],"_subset_german_step25.bim",sep="")
  dat[i,3]<-paste(path,"pre_imputation/QC/relatedness/",cohorts[i],"_subset_german_step25.fam",sep="")
}

write.table(dat,paste(path,"pre_imputation/QC/relatedness/list_cohorts_tomerge_german_step25.txt",sep=""),col.names=F,row.names=F,quote=F,sep="\t")
...
/path/to/software/plink_linux_x86_64_20181202/./plink \
--bfile ${path_gwas}pre_imputation/QC/relatedness/kiel_austria_sibdcs_gsa_subset_german_step25 \
--allow-no-sex \
--merge-list ${path_gwas}pre_imputation/QC/relatedness/list_cohorts_tomerge_german_step25.txt \
--make-bed --out ${path_gwas}pre_imputation/QC/relatedness/german_merged_step25

# 67623 variants and 17453 people pass filters and QC.
# Among remaining phenotypes, 10847 are cases and 6606 are controls.
...
-e ${path_gwas}pre_imputation/QC/relatedness/logs/stderr_king_german_step25 \
-o ${path_gwas}pre_imputation/QC/relatedness/logs/stdout_king_german_step25 \
"/path/to/software/./king \
-b ${path_gwas}pre_imputation/QC/relatedness/german_merged_step25.bed \
--related --cpus 8 --prefix ${path_gwas}pre_imputation/QC/relatedness/german_merged_step25"
...
famall<-read.table(paste(path,"pre_imputation/QC/relatedness/german_merged_step25.fam",sep=""),head=F)
...
#                   Group.1  x.1  x.2
# 1              franke_gsa  430  427
# 2   german_affy6_old_gwas 1734 1027
# 3 kiel_austria_sibdcs_gsa 4442 9393

colnames(dat)[5]<-"sex"
aggregate(as.factor(dat$sex), by=list(dat$cohort), FUN=summary)
#                   Group.1  x.0  x.1  x.2
# 1              franke_gsa   18  485  354
# 2   german_affy6_old_gwas    3 1345 1413
# 3 kiel_austria_sibdcs_gsa  203 6757 6875

table(dat$V1==dat$V2)
# TRUE 
# 17453

kin<-read.table(paste(path,"pre_imputation/QC/relatedness/german_merged_step25.kin0",sep=""),head=T)
table(kin$InfType)
...
x
#                         franke_gsa german_affy6_old_gwas
# franke_gsa                       0                     2
# german_affy6_old_gwas           11                     0
# kiel_austria_sibdcs_gsa         89                    73
# 
#                         kiel_austria_sibdcs_gsa
# franke_gsa                                    0
# german_affy6_old_gwas                        81
# kiel_austria_sibdcs_gsa                       1
...
#                         franke_gsa german_affy6_old_gwas
# franke_gsa                       0                    13
# german_affy6_old_gwas           13                     0
# kiel_austria_sibdcs_gsa         89                   154
# 
#                         kiel_austria_sibdcs_gsa
# franke_gsa                                   89
# german_affy6_old_gwas                       154
# kiel_austria_sibdcs_gsa                       2


write.table(x,paste(path,"pre_imputation/QC/relatedness/german_dup_summary_table_step25.tsv",sep=""),col.names=T,row.names=T,sep="\t",quote=F)
...
# prioritize new gwas over old germany
old_gwas<-c("german_affy6_old_gwas")
new_gwas<-c("kiel_austria_sibdcs_gsa","franke_gsa")
...
for (i in 1:length(dup_ids)) {
  ...
  if (nrow(dup[which((dup$FID1 %in% ids_tmp) | (dup$FID2 %in% ids_tmp)),])==n_possible_combinations ) {
    ...
    if ( (dim(table(data$sex))!=1 | dim(table(data$pheno))!=1) & all(data$sex %in% c(1,2))) {
      ...
      # B.1: if one sample from franchimont_gsa, keep Kiel
      
      if (any(data$cohort %in% c("kiel_austria_sibdcs_gsa"))) {
        
        keep_sample<-data$V1[which(data$cohort=="kiel_austria_sibdcs_gsa")]
        
        # if more than one new pair in new gwas, keep sample with largest call rate:
        if (length(keep_sample)>1) {
          keep_sample<-data[which(data$cohort=="kiel_austria_sibdcs_gsa"),]
...
# 1  sample_id   1     1   german_affy6_old_gwas 0.0113200     1
# 2           sample_id   1     2              franke_gsa 0.0004179     1
# 3   sample_id   1     1   german_affy6_old_gwas 0.0035450     2
# 4           sample_id   1     2 kiel_austria_sibdcs_gsa 0.0115400     2
# 5           sample_id   1     1 kiel_austria_sibdcs_gsa 0.0006235     3
# 6           sample_id   1     2              franke_gsa 0.0016540     3
# 7           sample_id   2     1 kiel_austria_sibdcs_gsa 0.0008716     4
# 8           sample_id   2     2              franke_gsa 0.0008933     4
# 9           sample_id   2     1 kiel_austria_sibdcs_gsa 0.0039960     5
# 10          sample_id   2     2              franke_gsa 0.0008771     5
...
write.table(data_remove,paste(path,"pre_imputation/QC/relatedness/german_data_remove_step25",sep=""),col.names=T,row.names=T,sep="\t",quote=F)
write.table(data_remove_tmp,paste(path,"pre_imputation/QC/relatedness/list_german_data_remove_step25",sep=""),col.names=T,row.names=F,sep="\t",quote=F)

write.table(data_inconsist,paste(path,"pre_imputation/QC/relatedness/german_data_inconsist_step25",sep=""),col.names=T,row.names=T,sep="\t",quote=F)
write.table(aggregate(as.factor(dat$pheno), by=list(dat$cohort), FUN=summary),paste(path,"pre_imputation/QC/relatedness/german_case_control_pre_per_study_relatedness_step25",sep=""),col.names=T,row.names=F,sep="\t",quote=F)
