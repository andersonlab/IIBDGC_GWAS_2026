...
for (i in 1:nrow(dat))  {
    ...
    # # load ld:
    # ld<-fread(paste0(path_gwas,"post_imputation/2022/analysis/conditional_analysis/eur/allarrays_chr",chr,"_",dat$IIBDGC_GWAS_index_variant[i],"_list_573_index_ld_r2_0.6.tags"),head=F)
    # colnames(ld)<-"MarkerName"
    
    # tmp<-tmp[which(tmp$eqtl_MarkerName %in% ld$MarkerName),]
    tmp<-tmp[which(tmp$eqtl_MarkerName==dat$IIBDGC_GWAS_index_variant[i]),]
    # print(dim(tmp))
    ...
    if(nrow(tmp)>0) {
        ...
        gwas<-gwas[which(gwas$MarkerName %in% tmp$eqtl_MarkerName),c("MarkerName","BETA","SE")]
        ...
        tmp$se_wald_ratio<-tmp$se/tmp$BETA_gwas

        dat$n_variants_in_ld_for_wr[i]<-nrow(tmp)
        dat$wald_ratio[i]<-tmp$wald_ratio[1]
        dat$se_wald_ratio[i]<-tmp$se_wald_ratio[1]
...
fwrite(dat,paste0(path_gwas,"post_imputation/2022/analysis/metaanalysis/coloc_results/tmp_coloc_filtered_by_gwas_r2_0.6_allpheno_with_beta_with_wald_ratio_",chr,"_",ph,"_only_gwas_lead.tsv"),
