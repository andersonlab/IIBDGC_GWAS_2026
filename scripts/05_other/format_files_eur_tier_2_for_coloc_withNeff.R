...
for (j in 1:length(pheno)){
  ...
  tmp<-fread(paste(path_gwas,"post_imputation/2022/analysis/metaanalysis/",pheno[j],"/",chr,"_",pheno[j],"_meta_eur_tier_2_noGC_PCs_firthse_formatted_A2_effect_and_alternative_with_per_sample_rate_and_avgA2freq_withNeff.txt.gz",sep=""),head=T)
  ...
  tmp<-tmp[which(tmp$HetPVal>1E-50),]
  print(nrow(tmp))
  ...
  write.table(tmp,paste(path_coloc,"GWAS_data/",pheno[j],"_chr",chr,"_summary_stats_metal_nogc_July2023_Neff_eur_tier2.tsv",sep=""),
